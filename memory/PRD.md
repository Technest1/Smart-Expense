...
- **Parsing**: Regex-only (Indian bank SMS/email formats) — no external AI dependency, zero ongoing API cost
...
- **b1. Push notifications for subscription reminders** — needs Expo push setup, testable only in the built app
- Merchant-learning (remember user category overrides)

## Tech stack
- Expo SDK 54 / React Native / expo-router / react-native-svg
- FastAPI + MongoDB (motor)
- Google OAuth (direct); parsing is regex-only, no LLM in the pipeline
